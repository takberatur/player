...
useSeoMeta({
  title: `Create Ad Vast Tag - ${setting.value?.site_name || 'Forge Player'}`,
  description: 'Create a new Ad Vast Tag',
  keywords:
    setting.value?.site_keywords || 'Forge Player, StreamVibe, Live Streaming',
  robots: 'noindex, nofollow',
  ogType: 'website',
  ogSiteName: setting.value?.site_name || 'Forge Player',
  ogTitle: `Create Ad Vast Tag - ${setting.value?.site_name || 'Forge Player'}`,
  ogDescription: 'Create a new Ad Vast Tag',
  ogImage: setting.value?.site_og_image || '/images/logo.png',
  twitterCard: 'summary_large_image',
  twitterTitle: `Create Ad Vast Tag - ${setting.value?.site_name || 'Forge Player'}`,
  twitterDescription: 'Create a new Ad Vast Tag',
...
const form = useForm<CreateAdVastForm>({
  ...
  tracking_url: `${window.location.origin}/ads/track`,
});
...
const submit = () => {
  const route = CustomAdVastTagController.store();
      ...
      <div class="flex-none px-4 py-4 sm:px-6">
        <div class="space-y-1">
          <h1 class="text-2xl font-bold tracking-tight sm:text-3xl">
            Create Custom Ad VAST Tag
          </h1>
          <p class="text-sm text-muted-foreground">
            Create new custom ad vast tag
          </p>
        </div>
        <div
