...
const videoDetail = ref<Video | null>(page.props.video || null);
const openDialogTmdbInfo = ref<boolean>(false);
const openDialogExampleList = ref<boolean>(false);

useSeoMeta({
  title: `Edit Video ${videoDetail.value?.title || ''} - ${setting.value?.site_name || 'Forge Player'}`,
  description: `Update Video ${videoDetail.value?.title || ''}`,
  keywords:
    setting.value?.site_keywords || 'Forge Player, StreamVibe, Live Streaming',
  robots: 'noindex, nofollow',
  ogType: 'website',
  ogSiteName: setting.value?.site_name || 'Forge Player',
  ogTitle: `Edit Video ${videoDetail.value?.title || ''} - ${setting.value?.site_name || 'Forge Player'}`,
  ogDescription: `Update Video ${videoDetail.value?.title || ''}`,
  ogImage: setting.value?.site_og_image || '/images/logo.png',
  twitterCard: 'summary_large_image',
  twitterTitle: `Edit Video ${videoDetail.value?.title || ''} - ${setting.value?.site_name || 'Forge Player'}`,
  twitterDescription: `Update Video ${videoDetail.value?.title || ''}`,
...
const form = useForm<CreateVideoForm>({
  ...
  original_link: videoDetail.value?.original_link || '',
  type: videoDetail.value?.type || 'google_drive',
  poster: videoDetail.value?.poster || '',
  ad_vast: videoDetail.value?.ad_vast || '',
  enable_popunder_ad: videoDetail.value?.enable_popunder_ad || false,
  popunder_ad_code: videoDetail.value?.popunder_ad_code || '',
  additional_ad_vast: videoDetail.value?.additional_ad_vast || '',
  enable_button_download: videoDetail.value?.enable_button_download || false,
  download_link: videoDetail.value?.download_link || '',
  subtitles:
    videoDetail.value?.subtitles?.map((s) => ({
      id: s.id,
      language: s.language,
      name: s.name,
      file: undefined as unknown as File, // File is optional for existing subtitles
    })) || [],
});
...
const submit = () => {
  ...
  const route = VideoController.update(videoDetail.value.id);
  ...
  form
    .transform((data) => ({
      ...
      subtitles: data.subtitles?.map((s) => ({
        ...s,
        file: s.file instanceof File ? s.file : undefined,
      })),
    }))
    .post(route.url, {
      preserveScroll: true,
...
const breadcrumbs: BreadcrumbItem[] = [
  {
    title: 'Videos',
    href: `/admin/video/${videoDetail.value?.id}`,
  },
];
</script>
...
<template>
  <Head>
    <title>Edit Video - {{ page.props.setting?.site_name ?? '' }}</title>
    <meta
      name="description"
      :content="`${page.props.setting?.site_description ?? ''} - Edit Video`"
    />
